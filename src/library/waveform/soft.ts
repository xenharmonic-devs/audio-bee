...
export function softSemisine(phase: number, sharpness: number) {
...
export function softSawtooth(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return sine(phase);
  }
  ...
  return (
    Math.atan((sharpness * sine(phase)) / (1 + sharpness * cosine(phase))) /
    Math.asin(sharpness)
  );
}

export function softTriangle(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return sine(phase);
  }
  if (sharpness >= 1) {
...
export function softSquare(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return sine(phase);
  }
  if (sharpness > 1 - EPSILON) {
...
export function softSinh(phase: number, sharpness: number, separation: number) {
  if (sharpness < EPSILON) {
    return cosine(phase);
  }
  return (
    (Math.exp(sine(phase + separation) * sharpness) -
      Math.exp(sine(phase - separation) * sharpness)) /
    ((Math.exp(sharpness) - Math.exp(-sharpness)) *
      sine(separation) *
      (1 + (0.25 - separation) * sharpness))
  );
}

export function softCosh(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return -cosine(phase);
  }
  return (
    (2 * (Math.cosh(Math.sin(Math.PI * phase) * sharpness) - 1)) /
      (Math.cosh(sharpness) - 1) -
    1
  );
}

export function softTanh(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return sine(phase);
  }
...
export function softLog(phase: number, sharpness: number) {
  if (sharpness < EPSILON) {
    return sine(phase);
  }
  if (sharpness > 1 - EPSILON) {
  ...
  return (
    2 *
      ((Math.log1p(sine(phase) * sharpness) - min) /
        (Math.log1p(sharpness) - min)) -
    1
  );
}
...
export function softPulse(
  ...
  return (
    ((Math.atan((a + b) / (1 + c - d)) - Math.atan((a - b) / (1 + c + d))) *
      (0.5 + sharpness * (TWO_PER_PI - 0.5))) /
    (sharpness * (ss + (1 - ss) * sharpness))
  );
...
export function softTent(phase: number, sharpness: number, separation: number) {
  if (sharpness < EPSILON) {
    return -sine(phase);
  }
  if (sharpness >= 1) {
    return (
      (0.5 * (semisine(phase + separation) - semisine(phase - separation))) /
      sine(separation)
    );
  }
